...
import { sanityClient, TeamMember } from '../../lib/sanity';
...
export default function TeamPage() {
  ...
  useEffect(() => {
    const fetchTeam = async () => {
      const data = await sanityClient.getTeam();
      setTeam(data);
    };
    fetchTeam();
  }, []);
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
            {team.map((member, index) => (
              <motion.div
                ...
                className="glass-panel p-5 rounded-2xl border border-white/5 flex flex-col group hover:border-white/10 transition-all duration-300 text-center"
              >
                {/* Photo card */}
                  ...
                  <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-end justify-center pb-4">
                    <div className="flex gap-3">
                      {member.socials.linkedin && (
                        <a href={member.socials.linkedin} target="_blank" rel="noreferrer" className="p-2 bg-accent rounded-lg text-black hover:bg-mint transition-colors">
                          <Linkedin className="w-4 h-4" />
                        </a>
                      )}
                      {member.socials.github && (
                        <a href={member.socials.github} target="_blank" rel="noreferrer" className="p-2 bg-accent rounded-lg text-black hover:bg-mint transition-colors">
                ...
                <span className="text-[10px] uppercase font-mono font-bold tracking-widest text-gray-500 mt-auto flex items-center justify-center gap-1">
                  <Code className="w-3 h-3 text-mint" /> Certified Architect
                </span>
