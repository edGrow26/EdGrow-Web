...
import { Sparkles, Calculator, CheckCircle2, ChevronDown, ChevronUp, Cpu, Sliders } from 'lucide-react';
...
export default function InteractiveCostCalculator() {
  // Feature flag to control calculator visibility
    ...
    <div id="cost-calculator-widget" className="glass-panel p-6 sm:p-10 rounded-3xl border border-white/10 relative overflow-hidden">
      ...
      <div className="flex items-center justify-between border-b border-white/5 pb-6 mb-8">
        ...
        <button
          ...
          className="p-2 bg-white/5 hover:bg-white/10 rounded-xl text-gray-400 hover:text-white transition-all flex items-center gap-2 text-xs font-semibold"
        >
          {showCalculator ? (
            <>
              Hide Widget <ChevronUp className="w-4 h-4" />
            </>
          ) : (
            <>
              Expand Calculator <ChevronDown className="w-4 h-4" />
            </>
          )}
        </button>
      </div>
